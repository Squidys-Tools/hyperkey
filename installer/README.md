...
The script reads the shared version from `Directory.Build.props`, publishes the app to `publish\win-x64`, and writes the installer to `publish\installer`. It requires the .NET SDK and Inno Setup 6. Packaging has not been validated yet; final install, upgrade, uninstall, and startup testing remains outstanding.
